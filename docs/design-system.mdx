...
| Context | Class | Size |
|---------|-------|------|
| Dark cards, outer | `rounded-3xl` | 24px |
...
**Constraints:** Never `border-`, `ring-`, `outline-` anywhere in the card tree. `rounded-3xl` on outer containers always. `zinc-800` outer → `zinc-900` inner is the entire separation mechanism. Status colors always use `/10` opacity backgrounds.
...
<Columns cols={2}>
  <Column>
    **Do**
    ...
    - `rounded-3xl` on all outer card containers
