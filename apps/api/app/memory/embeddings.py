...
async def _post_with_retry(client: httpx.AsyncClient, url: str, payload: dict) -> httpx.Response:
    """POST with fixed backoff until success, a non-retryable status, or the retry budget runs out.

    A 503 means the sidecar is overloaded, a connection error means it's
    mid-restart; dropping the memory operation over either would lose data.
    """
