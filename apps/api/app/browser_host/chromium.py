...
browser context: cookies and storage never cross contexts, yet a second
context costs a fraction of a second Chromium. All context lifecycle (create,
seed, dump, dispose) flows through one root CDP connection.
...
ChromiumHost owns:
  ...
  * crash recovery: a process-exit watcher relaunches a dead engine the instant
    ...
    matters most for Obscura, a from-scratch engine that can segfault the
    whole process on some pages, turning a multi-second outage into a blip.
"""
...
# Extra flags on top of browser-use's CHROME_DEFAULT_ARGS: never phone home
# for component updates, and the two flags a containerized Chromium needs.

# --no-sandbox is deliberate, not an oversight: the container lacks the user
# namespaces Chromium's renderer sandbox needs, so root refuses to run without
# it and a non-root uid fails under Docker's default seccomp.

# The real fix is an isolation boundary per session, not this flag. Until then,
# compensating controls: no data-plane network, downloads denied per context,
# and an http(s)-only navigation allowlist in the CDP proxy.
...
# response with no timeout of its own, so a wedged renderer would otherwise
# freeze whoever awaits it, session lock and reaper included, while looking alive.
...
async def cdp_call(
    ...
    *,
    ...
    """Run one CDP round-trip, bounded by timeout.

    The single place the host talks to Chromium, so a wedged browser always
    raises CDPTimeoutError instead of suspending its caller forever.
    """
    try:
...
def _resolve_chromium_path() -> str:
    ...
    The shell build drops the browser-UI layer (measured: 702 MB vs 1419 MB with three
    contexts open) and falls back to the full browser when absent. Playwright's resolver
    uses its sync API, so callers must invoke this in a worker thread.
    """
...
def _cdp_cookie_to_storage_state(cookie: dict[str, Any]) -> StorageStateCookie:
    """Convert a CDP Network.Cookie to a Playwright storage_state cookie."""
...
def _storage_state_cookie_to_cdp(cookie: StorageStateCookie) -> dict[str, Any]:
    """Convert a Playwright storage_state cookie to a CDP Storage.setCookies param."""
    out: dict[str, Any] = {
        "name": cookie["name"],
        "value": cookie["value"],
        "domain": cookie["domain"],
...
class ChromiumHost:
    ...
    the Obscura CDP server, selected by settings.BROWSER_ENGINE. Everything past
    launch, contexts, the CDP client, proxy, screencast, speaks plain CDP and is
    identical for both.
    """
    ...
    def __init__(self) -> None:
        ...
        # shutdown, not a crash. An Event, not a bool: the type checker narrows a
        # bool read after the await to a constant; .is_set() stays honest across it.
    ...
    async def stop(self) -> None:
        ...
        # before we tear down, instead of racing it. Once held, _stopping blocks
        # any new recovery, so cancelling the watcher next cannot interrupt a relaunch.
    ...
    async def create_context(self, storage_state: StorageState | None) -> HostSession:
        ...
        Fails fast with AtCapacityError once BROWSER_HOST_MAX_SESSIONS contexts
        are live, so a caller gets a clean 429 instead of a Chromium that slowly
        runs out of memory.
        """
        ...
        try:
            ...
            # Refuse downloads before the context can navigate anywhere: a drive-by
            # download is the cheapest way an attacker-influenced page gets a file
            # onto the host's disk. Scoped to this context to avoid racing another session.
            ...
            async with self._lock:
                ...
                # counted twice, giving a concurrent caller a spurious 429.
    ...
    async def dispose_context(self, session_id: str) -> StorageState:
        ...
        try:
            ...
            # registry forever, burning a slot after the caller had moved on.
            async with self._lock:
    ...
    def get(self, session_id: str) -> HostSession | None:
        """Return the live session, or None if unknown or disposed."""
    ...
    async def _reserve_slot(self) -> None:
        ...
        The gate is memory, not a session count: admit while projected usage stays
        under the high watermark, else wait for the reaper to free room before
        raising AtCapacityError. BROWSER_HOST_MAX_SESSIONS is only a runaway backstop.
        """
    ...
    async def _seed_local_storage(self, target_id: str, storage_state: StorageState) -> None:
        ...
        Registers a restore script per origin that seeds a key only if unset, safe
        to re-run and never clobbering a live update. Covers only the context's
        initial page target; a tab opened later via window.open is not restored.
        """
    ...
    def _obscura_command(self) -> list[str]:
        """Return Obscura's argv. It is a CDP server (serve), not a chrome debug flag.

        It publishes its DevTools endpoint at /json/version on the port we name
        (never ephemeral, so we can poll for it), stealthed, and permitted to
        reach the private network the host allowlist otherwise fronts.
        """
    ...
    async def _read_devtools_port(self) -> int:
        ...
        while time.monotonic() < deadline:
            ...
            if port_file.exists():
                ...
                # between the exists() check and the flush it can be empty. Read
                # defensively and keep polling instead of crashing on a partial write.
...
def _build_local_storage_restore_js(origin: str, entries: list[LocalStorageEntry]) -> str:
    ...
    each key only if absent so a value the page updated is never clobbered and
    the script is safe to re-run on every navigation.
    """
    return (
        "(() => {"
