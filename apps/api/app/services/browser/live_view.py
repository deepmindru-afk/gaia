...
(the card fetches a ?t= token because the host-only session cookie is not sent
cross-origin); a bot user, who has no web session, opens the same URL with a
?t= takeover token, which serves the self-contained HTML viewer below. Both
...
async def create_live_view_link(session_id: str, user_id: str) -> str:
    ...
    {vhost}/{code} when a dedicated live-view vhost is configured (the vhost
    rewrites /{code} to the app's /live/{code}), else {host}/live/{code}. The
    code maps to the session and owner in Redis; no session id or token in the URL.
    """
...
def render_live_view_page(session_id: str) -> str:
    ...
    same-origin session cookie), draws each JPEG frame onto a canvas, and
    forwards pointer/keyboard input as CDP-shaped mouse/key messages.
    """
