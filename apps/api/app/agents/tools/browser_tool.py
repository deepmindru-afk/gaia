"""The two executor-facing browser-automation tools: start a run, and collect it.

The executor sees a start and a join, not the internals. The "do you want me to
...
from app.templates.docstrings.browser_tool_docs import BROWSER_TASK, WAIT_FOR_BROWSER_TASK
...
async def wait_for_browser_task(
    ...
    try:
        return await _poll_job(job_id, params.conversation_id, stream_id, timeout)
    finally:
        # Every exit hands delivery back to the worker: it stays silent for as
        # long as this lease is held, so a leaked one loses the result entirely.
        await drop_joiner_lease(job_id)


async def _poll_job(job_id: str, conversation_id: str, stream_id: str, timeout: int) -> str:
    ...
    while True:
        ...
        if state is not None and state.status is BrowserJobStatus.DONE:
            return state.agent_message or agent_result_message(_DEAD_WORKER_RESULT)
        if await get_conversation_slot(conversation_id) is None:
            ...
            return agent_result_message(_DEAD_WORKER_RESULT)
        if waited >= timeout:
            return (
