...
async def test_run_cdp_proxy_connects_with_root_url_and_unbounded_kwargs() -> None:
    """Leave max_size and ping_interval unbounded, since a CDP screenshot frame can exceed the 1MB default and a ping interval would race the client's idle handling."""
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_decodes_real_bytes_frame_from_chromium() -> None:
    """Chromium's websocket may yield bytes frames; the proxy must decode them before filtering or forwarding, not pass raw bytes through."""
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_records_navigation_and_page_metrics_for_this_session() -> None:
    """Page.navigate opens a navigation, Target.createTarget counts a page, and Page.loadEventFired closes it, all attributed to this session id only."""
    import asyncio
    from unittest.mock import AsyncMock, MagicMock, call, patch
