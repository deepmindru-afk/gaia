...
def test_filter_downstream_scoped_event_missing_params_key_passes_through() -> None:
    """A scoped event with no ``params`` key at all (not merely an empty dict) must still resolve to ``raw`` without raising: ``_event_context_id`` is handed the default for a missing key, which must stay a dict, not ``None``."""
    ids: set[int] = set()
    for method in ["Target.attachedToTarget", "Target.targetCreated", "Target.targetInfoChanged"]:
...
async def test_run_cdp_proxy_touches_session_id_on_both_directions() -> None:
    """``host.touch`` must key off the session id — not the context id or anything else — on every frame in either direction, so the idle reaper leaves an in-use session alone."""
    import asyncio
...
async def test_run_cdp_proxy_rewrites_upstream_using_context_id_not_session_id() -> None:
    """``_rewrite_upstream`` must be called with ``session.context_id`` — pinning a new target to the session id (or anything else) would create the tab in the wrong browser context."""
    import asyncio
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_decodes_real_bytes_frame_from_chromium() -> None:
    """Chromium's websocket may yield ``bytes`` frames; the proxy must decode them before filtering/forwarding, not pass raw bytes through."""
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_logs_refusal_with_reason_and_session_id() -> None:
    """The refusal warning must carry the actual session id and the actual refusal reason, not a generic message."""
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_logs_closure_with_session_id_and_operation() -> None:
    """The closing log line must name this session and the ``cdp_proxy_closed`` operation, so a reader can tell which session's proxy exited from logs alone."""
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_never_forwards_dropped_downstream_frame() -> None:
    """When every downstream frame is filtered out (``_filter_downstream`` returns ``None`` for all of them), the client socket must receive nothing at all."""
    import asyncio
    from unittest.mock import AsyncMock, MagicMock, patch
...
async def test_run_cdp_proxy_records_navigation_and_page_metrics_for_this_session() -> None:
    """The per-session metrics the host reports (navigation count, page count, navigation duration) are fed exclusively from this proxy: a ``Page.navigate`` opens a navigation, a ``Target.createTarget`` counts a page, and the downstream ``Page.loadEventFired`` closes the navigation's timing.

    Each must be attributed to THIS session id, and nothing else may trigger them."""
    import asyncio
    from unittest.mock import AsyncMock, MagicMock, call, patch
