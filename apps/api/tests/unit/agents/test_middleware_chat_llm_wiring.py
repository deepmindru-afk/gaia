...
def _spawnable_tool(query: str) -> str:
    """Stand in for a real tool handed to spawned subagents."""
    return query
...
class TestCommsStackComposition:
    """Comms is the only tier whose text a person reads and the only one that delegates instead of acting."""

    def test_the_empty_completion_retry_is_the_innermost_middleware(self) -> None:
        """It has to see the raw completion; anything above it reads a reply the retry may still replace."""
    ...
    def test_comms_can_never_spawn_a_subagent(self) -> None:
        """Comms has no work tools by design — a spawn tool here would let the front door do the work."""
        stack = create_comms_middleware(chat_llm=_fake_llm())
...
class TestRequestConfigBinding:
    async def test_digest_is_invoked_with_the_request_configurable(self) -> None:
        """awrap_tool_call must bind the request's configurable onto the digest call, same as the model node."""
...
class TestStackConfigurationPropagation:
    ...
    def test_a_subagent_meters_under_its_own_name(self) -> None:
        """Without its own name, every one of ~35 integration subagents meters as provider_subagent."""
        from app.agents.middleware.accounting import LLMAccountingMiddleware
    ...
    def test_an_unnamed_subagent_still_meters_as_the_generic_bucket(self) -> None:
        """An unnamed sub-subagent stack keeps the generic bucket rather than crashing or going unattributed."""
        from app.agents.middleware.accounting import LLMAccountingMiddleware
...
class TestSpawnWiring:
    """Every SubagentStackOptions field must land on SubagentMiddlewareConfig, not silently fall back to a default."""
    ...
    def test_the_full_stack_is_this_exact_sequence(self) -> None:
        """Order is the contract: accounting outside, HIL gate before side effects, loop guard sees raw results."""
        stack = create_middleware_stack(
            chat_llm=_fake_llm(),
    ...
    def test_the_spawn_factory_builds_a_child_that_cannot_spawn_again(self) -> None:
        """A sub-subagent must not get spawn rights of its own, or a spawn tree can recurse without bound."""
    ...
    def test_the_spawn_factory_hands_the_child_its_space(self, monkeypatch) -> None:
        """The child stack is built for the space the spawn runs in, not a default."""
        from app.agents.middleware import factory as factory_mod
...
class TestExecutorStackComposition:
    """The executor is the only tier that both spawns subagents and must collect them."""
    ...
    def test_the_executor_compacts_everything_but_the_self_bounded_tools(self) -> None:
        """The exclusion set is a union: coding tools are capped, spawn_subagent digests, offloaders write their own format."""
        stack = self._executor(_fake_llm(), _fake_llm(), ToolRuntimeConfig(), {})
        compactor = next(mw for mw in stack if isinstance(mw, WorkspaceCompactionMiddleware))
...
class TestCommsAndSubagentDelegation:
    def test_comms_delegates_the_exact_options(self, monkeypatch) -> None:
        """Comms never spawns and never compacts, both off by explicit value, not by accident of a default."""
        captured = _spy_on_stack(monkeypatch)
        llm = _fake_llm()
    ...
    def test_a_subagent_summarizes_its_own_history(self) -> None:
        """Without summarization, production subagent runs averaged 91k input tokens per call against 43k elsewhere."""
        llm = _fake_llm()
