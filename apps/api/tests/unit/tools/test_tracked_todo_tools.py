...
class TestPatchCanvasSection:
    def test_replaces_a_section_that_is_the_first_line_of_the_canvas(self):
        """Regression: a freshly-created canvas's first section (no leading blank line before its heading) must be replaced in place, not duplicated at the end."""
...
class TestFormatCreateOutput:
    def test_the_summary_routes_canvas_edits_away_from_filesystem_tools(self) -> None:
        """Without this line the model reaches for the file tools, edits nothing the todo can see, and reports success — the sentence is the guardrail, pinned verbatim."""
        now = datetime.now(UTC)
