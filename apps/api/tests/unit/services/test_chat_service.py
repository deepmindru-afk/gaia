...
class TestChunkDispatchAndBackstop:
    """Frame routing and the executor-card backstop this branch adds to the stream."""

    # ── chunk dispatch ────────────────────────────────────────────────
    # Every frame goes through `_dispatch_stream_chunk`; the tests above only assert
    # the happy text path, so these pin the error / thinking / passthrough branches.
    ...
    # The attach owns the executor's cards on a live delegated turn; `_finalize_stream`
    # re-runs it as a backstop when the turn was cut short. Both paths fail silently.

    async def test_a_completed_attach_is_not_repeated_by_the_finally_backstop(
        self, test_user
    ) -> None:
        """The attached flag is what makes the backstop a backstop."""
