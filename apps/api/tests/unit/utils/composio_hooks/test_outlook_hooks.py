...
class TestOutlookAttachmentBeforeHook:
    ...
    def test_plain_http_url_is_rejected(self):
        # An http:// attachment would be fetched by Composio's staging flow over
        # cleartext (CWE-319), so reject it rather than forward it. https:// and
        # workspace paths remain fine.
        arguments = {"attachment": "http://drive/download/1"}
        params = {"arguments": arguments, "user_id": "u1"}
        with (
            patch(f"{HOOKS}.mint_share_url") as mint,
            pytest.raises(HookAbortError, match="cleartext"),
        ):
            outlook_attachment_before_hook("OUTLOOK_SEND_EMAIL", "outlook", params)
        assert mint.called is False

    def test_needs_grant_distinguishes_paths_from_urls(self):
        # Only a non-URL path needs minting; either scheme (and non-strings) do not.
        assert _needs_grant("/workspace/a.pdf") is True
        assert _needs_grant("https://x/a.pdf") is False
        assert _needs_grant("http://x/a.pdf") is False
        assert _needs_grant(123) is False
