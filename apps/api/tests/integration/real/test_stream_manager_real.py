...
class TestStreamManagerReal:
    ...
    async def test_publish_and_subscribe(self, real_redis):
        ...
        async def publisher():
            # No settle needed: subscribe_stream replays the event log from its
            # start and processes entries in insertion order.
    ...
    async def test_complete_stream_terminates_subscriber(self, real_redis):
        ...
        async def publisher():
            # The ghost chunk lands after DONE in the event log; the subscriber
            # stops at the DONE entry and never reaches it.
