...
def pro_user() -> AuthenticatedUser:
    """Authenticated user for a paying PRO user.

    FAKE_USER-shaped; ``PRO_USER_SUBSCRIPTION`` is the real
    ``UserSubscriptionStatus`` for its PRO plan. The global
    ``get_user_subscription_status`` patch always reports FREE, so PRO-tier
    tests patch that seam with ``PRO_USER_SUBSCRIPTION``.
    """
    return PRO_USER
